...
## [WPI] 세션 14 인계 (2026-04-15)

### 시작 상태
- main=`ed958a1`. 세션 13 브랜치 `claude/session-11-a7-i3` HEAD=`684737d` (코드 커밋 4 + docs 커밋 1), origin 푸시 완료.
- **PR 아직 미생성**: 세션 13 환경에 `gh` CLI 미설치로 PR 자동 생성 불가. 세션 14 첫 턴에 `gh pr create` 또는 GitHub 웹 UI 에서 `claude/session-11-a7-i3 → main` PR 수동 생성 필요. PR 본문은 아래 "세션 13 산출물" 섹션의 5개 커밋 설명을 그대로 사용하면 됨.
- 세션 13 에서 C-1 / C-2 / CI alembic roundtrip / A-6·A-7 리뷰 피드백 4건 처리. pytest 25 green.
- 세션 13 말 code-reviewer 라운드 남긴 non-blocker 잔재 I1/I2/N1/N2/N3 (위 참조).
...
**0. PR 생성** — 위 "시작 상태" 참조. `gh pr create --base main --head claude/session-11-a7-i3 --title "session 13: C-1/C-2/CI roundtrip + A-6/A-7 reviewer fix-ups"` 실행 후 CI 통과 대기.

**1. 세션 13 PR 머지 확인 + main rebase** — CI green 확인 후 squash merge. `git checkout main && git pull origin main` 으로 시작.

**2. 세션 13 리뷰 잔재 픽업 (API-free, 10분)** — 새 브랜치에서 I1 + I2 + N2 한 커밋으로 처리. N1 (`publish_matchups` bulk UPDATE) 는 진짜 문제 생길 때까지 defer, N3 는 `Matchup` 모델에 `updated_at` 실존 확인 후 결정.
   - **I1 SR_VARIANTS 라벨 리네임** — `scripts/verify_srid.py` `"claude_md_spec (0,9,6)"` → `"pre_session11_guess (0,9,6)"` (또는 유사). 스펙값이 `0,1,3,4,5,7` 라는 걸 라벨만 봐도 명확하게.
   - **I2 CI roundtrip sanity assertion** — `.github/workflows/ci.yml` 의 roundtrip 스텝에 `alembic upgrade head 2>&1 | tee /tmp/upgrade.log && grep -q "Running upgrade" /tmp/upgrade.log` 류 assertion 추가. 또는 `alembic current` 로 revision 확인.
   - **N2 `_one` 중복 sleep** — `verify_srid.py` `_one` 입장 sleep 또는 `main` 루프 뒤 sleep 중 하나 제거 (첫 호출 안전성 고려해 `_one` 입장 1회만 유지 권장).

**3. Phase 6 실 배포** (Vercel FE + Railway/Fly BE) — I3 가드/I2 uid 가이드/세션 13 C-1·C-2 필터 버그 fix 전부 준비됨. 이제 진짜 배포 가능:
...
**4. Docker 실 smoke** — 로컬 `docker` CLI 설치 후 `docker compose up` 엔드투엔드. I2 uid 가이드 준수.

**5. Phase 6 면책 copy review** — API 불필요, 정적 텍스트. `frontend/src/components/Footer.tsx` + OG route + about 영역 "엔터테인먼트 목적 · 베팅 무관" 최종 톤.

**6. 포스트시즌 재검증 여지** — 변동 없음. 가을 도달 시 `scripts/verify_srid.py` 재실행.
