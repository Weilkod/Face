...
| D-8 (360px 모바일 smoke test) | ⚠️ partial (세션 7) | `next dev` 구동 + `/`, `/pitcher/1`, `/history` curl 200 OK 확인. 컴포넌트 정적 분석 (MatchupCard/ShareButton/Footer/RadarChart) — 하드코딩 360 초과 너비 0건, `min-h-[44px]` 터치 타겟 보존, `viewBox`+`w-full` radar 스케일 OK. 실제 DevTools 360×800 육안 검수는 헤드리스 환경에서 불가 → 사용자 로컬 재검 필요 |
...
- [x] **D-8** (partial) dev 서버 + 3 경로 200 + 정적 분석 통과. 360px 육안 검수는 사용자 로컬 재검 필요 (세션 7)
