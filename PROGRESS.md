...
- [x] **C-1** `_append_review` dedup + 24h TTL + admin `/admin/review-queue` GET/resolve 엔드포인트 — Wave 2 Track F v2 (PR #24). fcntl/DB 승격 동시성 안전은 Postgres 전환 시 재검토 (별도 과제).
- [x] **C-2** `publish_matchups` — `.where(Matchup.is_published.is_(False))` 필터 추가 (Wave 1 Track A, PR #16)
...
### Wave 2 Track E 실행 결과 (2026-04-16, 브랜치 `claude/wave2-track-e-fe-live-integration-a4a6a3dc`)
...
| `/api/today` 응답 | ✅ 2개 matchup 반환 (SAM vs DS, LG vs KIA) |
...
| `npm run type-check` | ✅ clean (2회 확인) |
| `npm run build` | ✅ clean (2회 확인) |
| `/` 페이지 렌더 | ✅ 원태인/곽빈/손주영/양현종 카드 렌더 |
| `/history?date=2026-04-16` 렌더 | ✅ 2개 카드 + 적중률 통계 |
| `/pitcher/1` 렌더 | ✅ 원태인 프로필 + 관상/운세 점수 + 레이더 차트 |
| Footer 면책 고지 (3 페이지) | ✅ "엔터테인먼트 목적" 모든 페이지 확인 |
| Empty state (/ 경기 없는 날) | ✅ "오늘 예정된 매치업이 없습니다" + "과거 매치업 보기" 링크 |
| Empty state (/history 빈 날짜) | ✅ "해당 날짜에 경기 기록이 없습니다" |
| API 실패 에러 배너 + 재시도 | ✅ `ErrorBanner` 컴포넌트 + "다시 시도" reload 버튼 |
...
| 파일 | 변경 내용 |
|------|-----------|
| `frontend/.env.local` | 신설: `NEXT_PUBLIC_USE_MOCK=false` + `NEXT_PUBLIC_API_URL=http://localhost:8000` |
| `frontend/.env.example` | 신설: 환경변수 설명 템플릿 |
| `frontend/src/lib/api.ts` | `fetchJson` — 네트워크 오류를 `"fetch failed: ..."` 로 감싸서 throw (기존 HTTP 오류 throw 유지) |
| `frontend/src/components/ErrorBanner.tsx` | 신설: "use client" 배너 + reload 버튼, `isApiDown` prop으로 메시지 분기 |
| `frontend/src/app/page.tsx` | `ErrorBanner` 연동, Empty state에 "⚾ 경기 없는 날" + "과거 매치업 보기" 링크 추가 |
| `frontend/src/app/pitcher/[id]/page.tsx` | `ErrorBanner` 연동 (404/5xx 구분) |
| `frontend/src/app/history/page.tsx` | `ErrorBanner` 연동 |
| `frontend/src/components/MatchupCard.tsx` | `fetchDetail()` 분리 + 에러 시 "다시 시도" 버튼 추가 |
| `scripts/create_sample_matchup.py` | 신설: hash fallback으로 face/fortune/schedule/matchup row 2쌍 삽입 (is_published=True) |

#### Empty State 결정

- **홈 (`/`)**: 경기 없는 날 → "⚾" 아이콘 + "경기가 없는 날이거나 선발투수 발표 전일 수 있습니다. 선발투수는 보통 당일 오전 10~11시에 발표됩니다." + "과거 매치업 보기" 링크
- **히스토리 (`/history`)**: 빈 날짜 → "해당 날짜에 경기 기록이 없습니다." (기존 문구 유지, 날짜 선택 폼으로 다른 날 조회 가능)

#### API 실패 처리 결정

- `fetchJson` 는 네트워크 레벨 실패 시 `"fetch failed: ..."` 메시지로 throw — 기존 `!res.ok` throw 경로 보존
- `isApiDown` = `msg.includes("fetch failed")` 로 판별 → `ErrorBanner` 에서 "서버 연결 실패" + "데이터 서버에 연결할 수 없습니다." 표시
- Next.js ISR 캐시(`revalidate: 300`)가 있을 때는 stale 응답 서빙 — 이는 의도된 동작 (캐시 만료 후 재확인)
- `MatchupCard` 상세 fetch 실패 시 "다시 시도" 버튼으로 즉시 재시도 가능

#### 미해결 이슈

- **game_time null** — `/api/today` 에서 `game_time` 은 항상 null (today 라우터가 DailySchedule 미조회). 상세 펼치면 정상 노출됨 (matchup 라우터가 DailySchedule 조회). 스펙 OK (summary에는 game_time 없어도 무방).
- **chemistry.zodiac_detail/element_detail null** — matchup 라우터가 현재 ChemistryDetail 텍스트 필드를 채우지 않음 (DB 컬럼 미존재). 숫자 점수는 정상. Wave 3 Track A에서 Matchup 모델 확장 시 추가 가능.
- **hash fallback text** — face/fortune 텍스트가 "(폴백) 해시 기반 근사치" 로 표시됨 — 실 Claude 없이 시드했으므로 정상. 실 Claude 연동 후 갱신 필요.
- **360px 모바일 DevTools** — 자동화된 헤드리스 브라우저 없어 수동 확인 불가. 기존 D-8 Tailwind 클래스 구조 유지(변경 없음), `ErrorBanner`에 `min-h-[44px]` 터치 타깃 적용.

#### code-reviewer 라운드

Verdict: **APPROVE** — Critical 0, Important 0, Nits 1

Nit: `ErrorBanner` 에서 raw error message(`fetch failed: Failed to fetch`) 가 사용자에게 노출됨 → `isApiDown=true` 시 "데이터 서버에 연결할 수 없습니다." 로 대체 처리 완료.
