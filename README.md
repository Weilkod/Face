...
> **버전:** v1.2
> **작성일:** 2026-04-13 (최종 개정 2026-04-16)
> **상태:** Phase 1–7 구현 완료 · 프로덕션 배포 완료 (Railway BE + Vercel FE + Supabase Postgres). 프로덕션 DB 시딩은 후속 과제.
> **Prod URLs:** BE `https://face-production-0f00.up.railway.app` · FE `https://frontend-weilkods-projects.vercel.app`
...
| # | 항목명 | 내부 키 | 야구 의미 | 관상 요소 (10점) | 운세 요소 (10점) |
|---|--------|---------|-----------|-----------------|-----------------|
| 1 | **혜안력 (Command)** | `command` | 볼배합·코너워크·사사구 관리 | 눈매의 안정감·또렷함 | 오늘의 집중운·정밀운 |
| 2 | **결행력 (Stuff)** | `stuff` | 구속·무브먼트·삼진능력 | 턱·광대·하관의 강인함 | 오늘의 체력운·활력운 |
| 3 | **평정력 (Composure)** | `composure` | 위기관리·득점권 피안타율·페이스조절 | 이마 넓이·인중 깊이·미간 | 오늘의 정신운·인내운 |
| 4 | **상승운 (Dominance)** | `dominance` | 이닝소화·QS율·경기 지배 | 눈썹 기세·얼굴 균형·이목구비 비율 | 오늘의 승부운·권위운 |
| 5 | **운명력 (Destiny)** | `destiny` | 상대전적·구장궁합·팀운 | 전체 인상 조화도·얼굴형 | 오늘의 총운세·상성운 |

> **이름 변경 이력(2026-04-16):** 제구력→혜안력, 구위→결행력, 멘탈→평정력, 지배력→상승운. 내부 키(`command/stuff/composure/dominance/destiny`)·DB 컬럼·API 응답 스키마는 불변이므로 기존 `face_scores`/`fortune_scores` 캐시 재사용 가능.
...
#### ③ 평정력 (Composure) — "넓은 이마, 흔들리지 않는 마운드"
...
1. 혜안력 (Command) — 눈매의 선명도(4), 눈의 안정감(3), 눈-눈썹 간격(3)
2. 결행력 (Stuff) — 턱·하관 라인(4), 광대뼈(3), 코의 크기·높이(3)
3. 평정력 (Composure) — 이마 넓이·높이(4), 인중(3), 미간(3)
4. 상승운 (Dominance) — 눈썹 기세·농도(4), 좌우 균형(3), 이목구비 비율(3)
...
## 평가 항목
1. 혜안력 운세 — 오늘의 집중운, 정밀운 (10점)
2. 결행력 운세 — 오늘의 체력운, 활력운 (10점)
3. 평정력 운세 — 오늘의 정신운, 인내운 (10점)
4. 상승운 운세 — 오늘의 승부운, 권위운 (10점)
...
GET  /admin/review-queue?unresolved_only={bool}&limit={int}
     → 이름 매칭 실패 엔트리 조회 (크롤러가 쌓은 fuzzy 미매치)

POST /admin/review-queue/resolve
     → 수동으로 pitcher_id 매칭 후 resolved=True 플립
```

> ⚠️ **프로덕션 인증 gate 미적용.** 현재 `/admin/*` 은 dev/prod 양쪽에서 무인증 노출 중. `Depends(require_admin_token)` + `APP_ENV==prod` 조건부 gate 는 후속 과제(PROGRESS.md 후속 과제 참조).
...
```
[08:00] 크롤링 배치 (fetch_and_upsert_schedule)
...
[10:30] 분석 배치 (analyze_and_score_matchups)
        ├── 신규 투수 있으면 → 프로필 사진 수집 → 관상 분석
        ├── 전 선발투수 → 운세 생성
        └── 상성 계산 → 매치업 점수 산출 (per-game atomic 트랜잭션)

[11:00] 콘텐츠 발행 (publish_matchups)
        ├── `is_published = True` 플립 (미발행 matchup 만)
        ├── 프론트엔드 캐시 갱신 (revalidate: 300)
        └── (확장) SNS 자동 포스팅
...
**스케줄러 싱글톤 가드(프로덕션 필수):** `SCHEDULER_ENABLED` 환경변수로 다중 레플리카 중복 실행을 방지한다 (`app/main.py:lifespan`).
- **웹 서비스** (replicas ≥ 1): `SCHEDULER_ENABLED=false` — 트래픽 파드. `/admin/*` 수동 트리거는 `app.scheduler` 모듈 callable 을 직접 import 하므로 그대로 동작.
- **워커 프로세스** (replicas=1 고정): `SCHEDULER_ENABLED=true` — 크론 5잡 전용. 수평 확장 금지 (복제 시 `fortune_scores` 중복 write + Claude 토큰 2배).
- dev/staging 단일 프로세스는 기본값 유지.
...
║  └─────────────────┘ └─────────────────┘  ║
║                                           ║
║  💥 결행력 · Stuff     (17, 엔스)(12, 곽빈)║
║  (동일 포맷 — 관상/운세 설명 각 1줄)       ║
║                                           ║
║  🧘 평정력 · Composure (15, 엔스)(16, 곽빈)║
║  (동일 포맷)                               ║
║                                           ║
║  👑 상승운 · Dominance (18, 엔스)(15, 곽빈)║
║  (동일 포맷)                               ║
║                                           ║
...
| **배포 (FE)** | Vercel | Next.js 최적 배포, OG route Edge Runtime. Production URL `frontend-weilkods-projects.vercel.app` |
| **배포 (BE)** | Railway | FastAPI + Supabase Postgres (session pooler). `SCHEDULER_ENABLED=false` 로 다중 레플리카 안전. Production URL `face-production-0f00.up.railway.app` |
| **프로덕션 DB** | Supabase Postgres (free) + session pooler | `aws-1-ap-southeast-1.pooler.supabase.com:5432`. Direct connection 은 IPv6-only 라 Railway 에서 IPv4 pooler 필수 |
...
| `white` | `#ffffff` | 카드 배경 |
| `coral` | `#F26B4E` / light `#FEF3F0` / dark `#C54A30` | 홈팀, 혜안력·상승운 바, 프라이머리 액센트, 우승 카드 솔리드 배경 |
| `mint` | `#059669` / light `#D1FAE5` / dark `#065F46` | 원정팀, 평정력·결행력 바, 세컨더리 액센트 |
...
│   │   │   ├── ErrorBanner.tsx              # API 실패 배너 (isApiDown 분기 + reload)
│   │   │   └── Footer.tsx                   # 면책 고지 푸터
│   │   ├── lib/
...
│   ├── alembic.ini
│   ├── tests/                               # pytest (async, 151 tests)
│   │   ├── conftest.py                      # 임시 sqlite DATABASE_URL 주입
│   │   ├── test_analyze_rollback.py         # B-3 원자 트랜잭션 회귀 가드
│   │   ├── test_kbo_id_matcher.py           # A-5 id-first + lazy write-back
│   │   ├── test_chemistry_calculator.py     # D 상성 감사 (96)
│   │   ├── test_profile_harvester.py        # A-6 수확기 (9)
│   │   ├── test_publish_matchups.py         # C-2 미발행 필터
│   │   ├── test_review_queue.py             # C-1 dedup/TTL/admin (22)
...
│   ├── seed_pitchers.py                     # 투수 10명 시딩 (+ --harvest: A-6 eager KBO 수확)
│   ├── verify_ai_pipeline.py                # 실 Claude 파이프라인 캐시 미스/히트 검증 (B-1)
│   ├── verify_srid.py                       # KBO /ws/ srId 변종 라이브 비교 (A-7)
│   ├── crawl_today.py                       # GetKboGameList 단발 크롤 (--write 옵션)
│   ├── crawl_pitcher_images.py              # 프로필 사진 수확 (sync 레거시)
│   └── create_sample_matchup.py             # 해시 폴백으로 샘플 매치업 삽입 (FE smoke 용)
│
├── .claude/                           # 에이전트 정의 + 훅
...
| 크롤링 차단 (WAF IP-allowlist) | 일정/선발 데이터 수집 불가 — 2026-04-16 Wave 1 Track C 에서 Railway 미국 리전 IP 가 `403 Host not in allowlist` 블록 실측 | 한국 IP(로컬/residential/VPS) 에서 실행 · Playwright headless fallback(미구현) · 오프라인 fixture. 단일 소스 `koreabaseball.com` 정책 유지 (§7-2) |
...
> 세부 구현 로그는 `ARCHIVE.md`, 진행 상태는 `PROGRESS.md` 참조. 아래는 스펙 기준 스냅샷.

### Phase 1: 기반 구축 ✅ (2026-04-13)

- [x] DB 스키마 생성 및 Alembic 마이그레이션 (`0001_initial_schema`)
- [x] KBO 10개 구단 투수 마스터 데이터 구축 (10명 시드, `pitchers_2026.json`)
- [x] 투수 프로필 사진 수집 (manifest 기반)
- [x] 띠 궁합표(`zodiac_compatibility.json`), 별자리 원소(`constellation_elements.json`)

### Phase 2: AI 엔진 ✅ (2026-04-13, 실검증 2026-04-14)

- [x] Claude Vision 관상 분석 (`face_analyzer` + `face_analysis.txt`)
- [x] Claude Text 운세 생성 (`fortune_generator` + `fortune_reading.txt`)
- [x] 점수 캘리브레이션 + `hash_fallback` 해시 폴백
- [x] 상성 계산 모듈 (`chemistry_calculator`, 96 테스트)
- [x] 종합 스코어링 엔진 (`scoring_engine`, caller-managed transaction)

### Phase 3: 크롤러 + 배치 ✅ (2026-04-13)

- [x] `koreabaseball.com /ws/Main.asmx/GetKboGameList` 단일 소스 크롤러
- [x] 선발투수 매칭: playerId → 이름 exact → rapidfuzz fuzzy(≥85) → 리뷰 큐
- [x] `pitchers.kbo_player_id` + lazy write-back (Alembic `0002`, A-5)
- [x] `seed_pitchers.py --harvest` eager 프로필 수확기 (A-6, 10/10 hit)
- [x] APScheduler 5 KST 잡 (08/09/10/10:30/11:00) + `SCHEDULER_ENABLED` 싱글톤 가드
- [ ] 경기 결과 크롤러 (적중률 추적용) — 미구현, `/admin/update-result` 수동 입력만

### Phase 4: API 라우터 ✅ (2026-04-13, 세션 4)

- [x] 5 GET: `/api/today`, `/api/matchup/{id}`, `/api/pitcher/{id}`, `/api/history`, `/api/accuracy`
- [x] 7 POST admin: crawl-schedule, analyze-face, generate-fortune, calculate-matchups, update-result, review-queue(GET), review-queue/resolve
- [x] `predicted_winner` boundary resolve (DB `"home"/"away"` → response 한글 이름)

### Phase 5: 프론트엔드 ✅ (2026-04-13~)

- [x] Next.js 14 App Router + Tailwind + coral/mint/ink 라이트 팔레트
- [x] `MatchupCard` 아코디언, `RadarChart` 5축, `AxisDetail`, `ShareButton`, `ErrorBanner`, `Footer`
- [x] 메인 히어로 + `/history` + `/pitcher/[id]` (PitcherDetail 통합)
- [x] 실 API 연동 (`USE_MOCK=false`), TanStack Query, SSR revalidate 300
- [x] Empty state + API 실패 ErrorBanner + 360px 모바일 smoke
- [x] 공유 카드 `@vercel/og` Edge route (1200×630)
- [x] 면책 고지 footer (3 페이지 + OG route)

### Phase 6: 배포 인프라 ✅ (2026-04-13~14)

- [x] Alembic 단일 진실 원천, `init_db.py` 가 `upgrade head` 위임
- [x] Docker (`backend` slim+tini, `frontend` standalone) + `docker-compose.yml`
- [x] `.github/workflows/ci.yml` — pytest + alembic upgrade + frontend type-check/build

### Phase 7: FE↔BE 통합 & 런치 ✅ (2026-04-16, 4 Wave)

- [x] **Wave 1** — BE 스키마 G2/G3/G4, FE api.ts G1, 상성 감사(96테스트). 크롤 smoke 는 WAF IP-block 으로 FAIL (인프라).
- [x] **Wave 2** — FE 실 데이터 연동 (ErrorBanner/Empty state), 리뷰 큐 dedup/TTL/admin endpoints (22테스트).
- [x] **Wave 3** — 전체 diff 리뷰(Critical 3/Important 4 fix, 151 tests) + E2E pipeline smoke.
- [x] **Wave 4** — Docker compose smoke, Railway BE 배포, Vercel FE 배포, 면책 copy review.

### 후속 과제 (non-blocker)

- [ ] **프로덕션 DB 시딩** — Supabase Postgres 가 비어 empty-state 렌더. `scripts/seed_pitchers.py --harvest` + 관상/운세 실호출 + 샘플 matchup 생성 필요. 한국 IP 필수(KBO WAF).
- [ ] **`/admin/*` 인증 gate** — `Depends(require_admin_token)` + `APP_ENV==prod` 조건부. 특히 `/admin/review-queue/resolve` 조기 차단 권장.
- [ ] **크롤 WAF 언블록** — 한국/미국 residential IP 재실행 · Playwright headless fallback · 오프라인 fixture 중 선택.
- [ ] **워커 서비스 분리** — Railway 에 `worker` 서비스 신설 후 `SCHEDULER_ENABLED=true` + replicas=1 고정.
- [ ] **`I-G1 create_sample_matchup.py` chemistry placement** — 샘플 스크립트가 destiny axis total 에 chem 을 안 박아서 radar 수치가 프로덕션과 미묘히 다름.
- [ ] **`I-G2 DATABASE_URL` 상대경로 취약성** — `./data/facemetrics.db` 가 cwd 의존. 반드시 프로젝트 루트에서 기동.
- [ ] **포스트시즌 `srId` 재검증** — A-7 은 정규시즌 날짜만 검증됨. 가을 도달 시 `scripts/verify_srid.py` 재실행.
- [ ] **경기 결과 크롤러** — 적중률 자동 추적용 (현재 `/admin/update-result` 수동 입력만).
